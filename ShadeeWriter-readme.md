...
1.  **Guardrail logic** for detecting prompt attacks or attempts to force unsafe content.
2.  **Strict compliance instructions** that prevent harmful advice or factual errors in mental health contexts.
...
```mermaid
graph TD
    UI[User Input] --> TI[Topic Interpretation]
    TI --> SS[Structure Selector]
    SS --> TSL[Tone and Safety Layer]
    TSL --> CG[Content Generation]
    CG --> SC[Sensitivity Check]
    SC --> FA[Final Draft Assembly]
```
